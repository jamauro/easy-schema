...
Meteor.startup(() => {
  writeMethods.forEach(methodName => {
    ...
    Mongo.Collection.prototype[methodName] = async function(...args) {
      ...
      const { defaults, rules } = schema[meta] || {};
      const schemaToCheck = isUpdate && !isReplace ? _schemaDeepOptional : schema;
      if (isUpdate && rules) {
        schemaToCheck[meta].rules = rules; // / rules is only on schema but we want to put them on _schemaDeepOptional when we pass it into check
      }
