...
import { _rules, _defaults } from '../shape';
...
Meteor.startup(() => {
  writeMethods.forEach(methodName => {
    ...
    Mongo.Collection.prototype[methodName] = async function(...args) {
      ...
      const defaults = schema[_defaults]; // defaults is only on schema as an optimization
      const schemaToCheck = isUpdate && !isReplace ? _schemaDeepOptional : schema;
      if (isUpdate && schema[_rules]) schemaToCheck[_rules] = schema[_rules]; // rules is only on schema but we want to put them on _schemaDeepOptional when we pass it into check
