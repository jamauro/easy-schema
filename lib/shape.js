...
export const _extra = Symbol('_extra');
export const _rules = Symbol('_rules');
export const _defaults = Symbol('_defaults');
 ...
 */
export const shape = (schema, { optionalize = false } = {}) => {
  ...
  if (allowExtra(result)) {
    result[_extra] = true;
    delete result['0'];
  }

  rules.length && (result[_rules] = rules);
  defaults.length && (result[_defaults] = defaults);
