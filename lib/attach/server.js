...
Mongo.Collection.prototype.attachSchema = function(schema) {
  try {
    ...
    collection.schema = shape(fullSchema)
    collection.schema.$id = `/${collection._name}`;

    /** @type {import('meteor/check').Match.Pattern} */
    collection._schemaDeepOptional = shape(fullSchema, {optionalize: true});
    collection._schemaDeepOptional.$id = `/${collection._name}`;
