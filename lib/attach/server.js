...
import { shape, unwrap, allowExtra, isArray, ALLOWED, ID_PATTERN, Any } from '../shape.js';
import { isObject } from '../utils/shared';
import './both';

// adding for backwards compatability pre Meteor 3.0.2. can probably be removed one day to just use Mongo.getCollection only
const hasGetCollection = Mongo.getCollection;
const Collections = !hasGetCollection && new Map();
...
export const createJSONSchema = (obj) => {
  let optionalKeys = [];
  const additionalProperties = allowExtra(obj);
  if (additionalProperties) delete obj['0'];
    ...
    const property = (() => {
      if (optional) {
        ...
        if (Object.keys(conditions).some(i => !ALLOWED.includes(i))) { // this prevents a situation where the user has a {type: } as part of their schema but did not intend to use it to create conditions
          ...
          const { value: typeValue, optional, extra } = unwrap(type);
          if (optional) optionalKeys = [...optionalKeys, k];
          if (extra) conditions.extra = true;
        ...
        const { value: firstValue, optional, anyOf } = unwrap(value[0]);
        const { type: fvType, ...conditions } = firstValue; // might be using [{type: }]
        const { value: typeValue, optional: fvTypeOptional } = unwrap(fvType);
        ...
        const result = createJSONSchema(value);
        if (extra) result.additionalProperties = true;
...
Mongo.Collection.prototype.attachSchema = function(schema) {
  try {
    ...
    collection.schema = shape(fullSchema, { name })

    /** @type {import('meteor/check').Match.Pattern} */
    collection._schemaDeepOptional = shape(fullSchema, { name, optionalize: true });

    if (!hasGetCollection) { // adding for backwards compatability pre Meteor 3.0.2. can probably be removed one day to just use Mongo.getCollection only
