...
const attachMongoSchema = async (collection, schema) => {
  try {
    ...
    if (!collectionNames.includes(collection._name)) {
      await collection.insertAsync({ _id: 'setup schema' }, { autoCheck: false });
      Package['jam:soft-delete'] ? await collection.removeAsync({ _id: 'setup schema' },  { soft: false }) : await collection.removeAsync({ _id: 'setup schema' });
    }
